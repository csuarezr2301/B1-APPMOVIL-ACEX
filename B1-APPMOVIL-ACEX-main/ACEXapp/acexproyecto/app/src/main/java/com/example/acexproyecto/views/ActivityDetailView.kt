...
fun Observaciones(actividad: ActividadResponse?) {
    var observaciones by remember { mutableStateOf("Cargando observaciones...") } // Estado inicial
    var isDialogVisible by remember { mutableStateOf(false) }

    // Actualizamos observaciones cuando la actividad cambie
    LaunchedEffect(actividad) {
        if (actividad != null) {
            observaciones = actividad.incidencias.takeIf { it.isNotBlank() }
                ?: "No hay observaciones e incidencias"
        } else {
            observaciones = "No hay observaciones e incidencias"
        }
    }
    ...
    // Mostrar el contenido de las observaciones
    Text(
        text = observaciones,
        fontSize = 16.sp,
        color = TextPrimary
